...
void E32ImageFile::DumpHeader(TInt aDumpFlags)
{
	...
	{
		printf("V%d.%02d(%03d)", iOrigHdr->iToolsVersion.iMajor,iOrigHdr->iToolsVersion.iMinor,iOrigHdr->iToolsVersion.iBuild);
	...
	if(aDumpFlags&EDumpHeader)
	{
		...
		printf("Code    %06x %06x", OrigCodeOffset(), iOrigHdr->iCodeSize);

		if (iOrigHdr->iCodeRelocOffset)
		{
			E32RelocSection *r=(E32RelocSection *)(iData + iOrigHdr->iCodeRelocOffset);
			printf(" %06x %06x", OrigCodeRelocOffset(), r->iNumberOfRelocs);
		}
		else
			printf("              ");

		printf("        +%06x (entry pnt)", iOrigHdr->iEntryPoint);
		printf("\n");

		printf("Data    %06x %06x", OrigDataOffset(), iOrigHdr->iDataSize);

		if (iOrigHdr->iDataRelocOffset)
		{
			E32RelocSection *r=(E32RelocSection *)(iData + iOrigHdr->iDataRelocOffset);
			printf(" %06x %06x", OrigDataRelocOffset(), r->iNumberOfRelocs);
		}
		printf("\n");
